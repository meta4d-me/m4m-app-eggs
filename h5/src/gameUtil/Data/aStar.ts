...
class nodePool {
    ...
    public static new_node(x: number, y: number, G: number, H: number, p: node = null) {
    ...
    public static delete_node(n: node) {
        if (!n) { return; }
        n.parent = null;
...
export class aStar {
     ...
     */
    public outFilter: (x: number, y: number) => boolean;
    ...
    public findPath(start_x: number, start_y: number, end_x: number, end_y: number, outPath: number[]): boolean {
        let result = false;
    ...
    private findAddNeighbor(n: node) {
        ...
        for (let i = 0; i < this.cupPoints.length; i++) {
